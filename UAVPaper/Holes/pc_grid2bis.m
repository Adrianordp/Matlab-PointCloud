...
filename = 'pontos_1574700877_seg.asc'; %AQUI VC SELECIONA A NUVEM J� SEGMENTADA
xyz = load(filename); 
% xyz = xyz(:,1:3);
xy = xyz(:,1:2);
...
ptCloud = pointCloud(xyz_alinhado);
roi = [-500 -100 -100 -50 -200 200]; %Find the indices of the points that lie within the cuboid ROI.
indices = findPointsInROI(ptCloud,roi);
ptCloudB = select(ptCloud,indices);% Display the input point cloud and the point cloud within the specified ROI.

xyzf = ptCloudB.Location;
